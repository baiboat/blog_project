...
def global_setting(request):
    ...
    #浏览量排行
    article_lookcount_list=Article.objects.all().order_by('click_count')[:6]
    #站长推荐
    article_reco_list=Article.objects.filter(is_recommend='Ture').order_by('click_count')[:6]
    #用于广告页面
    article_ad_list0=article_reco_list[0]
    article_ad_list1 = article_reco_list[1]
    article_ad_list2 = article_reco_list[2]
    article_ad_list3 = article_reco_list[3]
    # 评论排行
...
def tag(request):
    try:
        ...
        try:
            ...
            return  render(request,'failure.html',{'reason':'该标签没有文章'})
...
def search(request):
    try:
        ...
        try:
            ...
            return  render(request,'failure.html',{'reason':'您搜索的文章不存在'})
    except Exception as e:
        logger.error(e)
    return render(request,'tag_article.html',locals())
